...
function library:Create(text, draggable, keybind)
	...
	function InsideLibrary:CreateTab(text)
		...
		local Section = {}
		function Section:RemoveTab()
			Container2:Destroy()
			tabButton:Destroy()
		end
		
		function Section:ClearTabElements()
		...
		function Section:CreateToggle(text, callback)
			...
			local togglefunc = {Enabled = default or false}

			if default then
				callback(togglefunc.Enabled)
			end
		...
		function Section:CreateSlider(args)
			local text = args.Text
		...
		function Section:CreateTextbox(args)
			local text = args.Text
			local callback = args.Function
